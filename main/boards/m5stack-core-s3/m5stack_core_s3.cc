...
private:
    ...
    int touch_last_x_ = 0, touch_last_y_ = 0;
    ...
    void InitializePowerSaveTimer() {
        ...
        // 注意：PowerSaveTimer 的 callback 不会立即执行，而是 sleep 超时才触发
        ...
        power_save_timer_->OnEnterSleepMode([this]() {
            ...
            // M5Stack Core S3 未进入 light sleep (cpu_max_freq = -1)，I2C 仍可用
            // 因此保留触摸轮询定时器，确保触摸可以唤醒设备
            // (如果停止定时器，PollTouchpad 不再运行 → WakeUp 永远无法被调用 → 死锁)
            // 注意：不暂停 motion_task_ / si12t_task_，因为 vTaskSuspend 可能在其持有
            // I2C 总线互斥锁时强制暂停，导致 I2C 死锁 → 触摸唤醒失效甚至系统 panic。
            // ESP-IDF v5 I2C 驱动自带总线锁，多任务并发访问是安全的。
        });
        power_save_timer_->OnExitSleepMode([this]() {
            if (!pmic_) return;  // 防止 pmic_ 未初始化
            // 只做轻量操作：标记退出休眠、恢复屏幕（不需要 I2C/UART）
            // 不恢复 motion_task_ / si12t_task_ — 它们从未被暂停，一直正常运行
            exit_sleep_pending_ = true;
            // 设置唤醒抑制期：从此刻起 2 秒内的触摸事件忽略，防止唤醒延续被识别为新点击
    ...
    void ResetTouchState() {
        was_touched_ = false;
        touch_start_time_ = 0;
        touch_start_x_ = 0; touch_start_y_ = 0;
        touch_last_x_ = 0; touch_last_y_ = 0;
        touch_total_move_ = 0;
        pet_triggered_ = false;
        pending_single_release_ = false;
        pending_single_release_time_ = 0;
    }

    void PollTouchpad() {
        // 唤醒后的抑制期内不处理任何触摸事件，避免唤醒延续被误识别为新点击
        int64_t now = esp_timer_get_time() / 1000;
        if (now < wakeup_grace_until_ms_) {
            // 抑制期结束后重置触摸状态，确保下一个触摸从干净状态开始
            if (wakeup_grace_until_ms_ - now <= 20) {
                ResetTouchState();
                wakeup_grace_until_ms_ = 0;
            }
            return;
        }
        ...
        // Update touch point (void, no error check needed)
        ft6336_->UpdateTouchPoint();
        ...
        // 检测到触摸时，先唤醒休眠模式（如果有），确保各外设恢复工作状态
        if (touch_point.num > 0 && power_save_timer_) {
            power_save_timer_->WakeUp();
            // 唤醒恢复尚未完成（I2C/UART 操作还在主循环排队），跳过这次触摸识别
            if (exit_sleep_pending_) {
                ResetTouchState();
                return;
            }
        }
    ...
    void InitializeSpi() {
        ...
        buscfg.miso_io_num = GPIO_NUM_35;  // SD-MISO（与 LCD-DC 复用，SD 操作时切输入）
...
public:
    M5StackCoreS3Board() {
        ...
        InitializeAxp2101();
        InitializeAw9523();
        InitializePowerSaveTimer();  // 移到 Axp2101 之后，确保 pmic_ 已就绪
        I2cDetect();
        ...
        avatar_display->SetLedUpdater([this](const char* emotion) {
            ...
            uint16_t color = 0;
            if (state == kDeviceStateSpeaking) {
                color = Rgb888To565(255, 120, 0);   // 说话：橙色
            } else if (state == kDeviceStateListening) {
